...
const Launch = props => {
  const selectorBody = useMemo(() => {
    return props.planets?.map(planet => 
      <option value={planet.planetName} key={planet.planetName}>{planet.planetName}</option>
    );
  }, [props.planets]);
  ...
  return <Appear id="launch" animate show={props.entered}>
    ...
    <form onSubmit={props.submitLaunch} style={{display: "inline-grid", gridTemplateColumns: "auto auto", gridGap: "10px 20px"}}>
      ...
      <input type="text" id="rocket-type" name="rocket-type" defaultValue="Explorer IS1" />
      <label htmlFor="destination-planet">Destination Exoplanet</label>
      <select id="destination-planet" name="destination-planet">
        {selectorBody}
      </select>
      <Clickable>
        <Button animate 
